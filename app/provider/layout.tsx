...
export default function ProviderLayout({
  ...
  const isProviderChat = pathname.startsWith("/provider/chat");
  ...
  const nav = [
    { href: "/provider", label: "Dashboard", Icon: Gauge },
    { href: "/provider/history", label: "Jobs", Icon: Wrench },
    { href: "/provider/earnings", label: "Metrics", Icon: ChartNoAxesCombined },
  ...
  return (
    <div className="h-dvh overflow-hidden lg:grid lg:grid-cols-[220px_1fr]">
      <aside className="hidden h-dvh overflow-y-auto border-r border-slate-300/70 bg-white/85 p-6 lg:flex lg:flex-col dark:border-white/10 dark:bg-[#101a2a]/95">
        <Link href="/provider" className="mb-8">
          <AutrifixLogo size="md" />
        </Link>
        <SidebarNavList items={nav} pathname={pathname} rootHref="/provider" />
      ...
      <main className="h-dvh overflow-hidden">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-300/70 bg-white/85 px-4 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-[#0B1F3A]/80">
          <Link href="/provider" className="lg:hidden">
          ...
          <div className="flex items-center gap-2">
            <Link href="/provider/navigate">
              <IconActionButton
                Icon={Route}
                label="Navigate"
                active={pathname.startsWith("/provider/navigate")}
              />
            </Link>
            <NotificationBell />
            <ThemeToggle />
        ...
        <div className={`${isProviderChat ? "h-[calc(100dvh-61px)] overflow-hidden" : "h-[calc(100dvh-61px)] overflow-y-auto pb-6"}`}>
          {children}
        </div>
      </main>
      {showSupportModal ? (
        ...
        >
          <GlassCard className="w-full max-w-xl border-slate-300/70 bg-white/95 p-5 dark:border-white/10 dark:bg-[#1a2437]/95">
            ...
            <div className="mt-4 grid gap-2">
              ...
              <a
                href="mailto:nathanielguggisberg@gmail.com?subject=Provider%20Support%20Request"
                className="rounded-xl border border-slate-300/70 bg-white px-3 py-2 text-sm text-slate-800 hover:bg-slate-50 dark:border-white/10 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10"
              >
                Email support
              </a>
