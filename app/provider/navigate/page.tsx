...
const CUSTOMER = { lat: 5.598, lng: -0.182 };
...
export default function ProviderNavigatePage() {
  ...
  const detailsHref = activeJob ? `/provider/job/${activeJob.id}` : "/provider/history";
  ...
  const customerName = target?.customer_name || "Customer";
  ...
  const fallbackCustomerName = activeJob?.customer_name || customerName;
  const fallbackIssueName = activeJob?.service_category_name || issueName;
  const dynamicRoute = useMemo(
    () => [
      mapCenter,
      {
        lat: (mapCenter.lat + customerPoint.lat) / 2,
        lng: (mapCenter.lng + customerPoint.lng) / 2,
      },
      customerPoint,
    ],
    [mapCenter, customerPoint],
  );
        ...
        markers={[
          {
            ...
            label: target ? customerName : fallbackCustomerName,
            subtitle: target ? `${vehicleSummary} · ${issueName}` : `Active job · ${fallbackIssueName}`,
          },
        ]}
        routePath={target ? dynamicRoute : PROVIDER_ROUTE}
