...
import { ProviderQuotePanel } from "@/components/jobs/provider-quote-panel";
import { GlassCard } from "@/components/ui/glass-card";
import { Button } from "@/components/ui/button";
import { CUSTOMER_VEHICLE_PACK_STORAGE_KEY, type VehiclePackSnapshot } from "@/lib/vehicle-profile";
import { finishJob, getJob, listJobs, startJob } from "@/services/jobs";
...
function readVehiclePack(): VehiclePackSnapshot | null {
  ...
  try {
    const raw = localStorage.getItem(CUSTOMER_VEHICLE_PACK_STORAGE_KEY);
    if (!raw) return null;
...
export default function ProviderJobPage() {
  ...
  const chatHref = id ? `/provider/chat/${id}` : "/provider/messages";
  const [pack, setPack] = useState<VehiclePackSnapshot | null>(null);
  const qc = useQueryClient();
  const jobsQ = useQuery({
    queryKey: ["jobs", "provider-job", id],
    queryFn: listJobs,
    staleTime: 10_000,
    enabled: Boolean(id),
  });
  ...
  // Finishing is only half of completion: it records the amount and hands the job to the
  // customer to confirm (backend ADR-022). The job does not close here.
  const finishMut = useMutation({
    mutationFn: (amount: string) => finishJob(id, amount),
    onSuccess: invalidateJob,
    onError: () => toast.error("Could not record the amount."),
  });
  ...
  const customerName = effectiveJob?.customer_name || "Customer";
  const issueLabel = effectiveJob?.service_category_name || "Roadside service";
  const statusLabel = jobStatusLabel(effectiveJob?.status);
  ...
  return (
    <div className="px-4 py-6">
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1fr_380px]">
        <div className="space-y-4">
          ...
          <GlassCard className="border-white/10 bg-[#253247]/90">
            ...
            <p className="mt-2 text-white/65">
              ...
              {v?.tire_size ? (
                <span className="mt-2 block text-[#8ef7bb]">Customer tire size on file: {v.tire_size}</span>
              ) : null}
            </p>
          </GlassCard>
        </div>
        <div className="space-y-4">
          <GlassCard className="border-white/10 bg-[#253247]/90">
            <p className="font-sora text-4xl text-white">{customerName}</p>
          ...
          <GlassCard className="border-white/10 bg-[#253247]/90">
            ...
            {pack?.readiness != null ? (
              <p className="mt-2 text-xs text-white/50">
                Fleet readiness score (customer primary):{" "}
            ...
            {!pack ? (
              <p className="mt-3 text-xs text-amber-200/80">
                No customer snapshot yet. When the customer saves vehicles at{" "}
                <span className="text-white">/customer/vehicles</span>, this card fills from local preview storage
          ...
          <GlassCard className="border-white/10 bg-[#253247]/90">
            <p className="text-[11px] uppercase tracking-[0.16em] text-white/45">Customer notes</p>
          ...
          {effectiveJob ? (
            <div className="mb-3">
              <ProviderQuotePanel job={effectiveJob} />
            </div>
          ) : null}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button
              ...
              disabled={startMut.isPending || effectiveJob?.status !== "pending_accept"}
            >
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (!/^\d+(\.\d{1,2})?$/.test(amount) || Number(amount) <= 0) {
                  toast.error("Enter the amount owed, e.g. 250.00");
                  return;
                }
                await finishMut.mutateAsync(amount);
                toast.success("Amount sent to the customer to confirm.");
              }}
              disabled={finishMut.isPending || effectiveJob?.status !== "active"}
            >
              Finish job
            </Button>
          </div>
          {effectiveJob?.status === "active" ? (
            <label className="mt-3 block">
              ...
              <span className="mt-1 block text-xs text-white/45">
                The customer confirms this before the job closes. Collect payment directly.
              </span>
            </label>
          ) : null}
          {effectiveJob?.status === "awaiting_confirmation" ? (
            <p className="mt-3 text-sm text-white/60">
              Waiting for the customer to confirm{" "}
              <span className="text-white/90">
                {effectiveJob.currency} {effectiveJob.final_amount}
              </span>
              .
            </p>
          ) : null}
