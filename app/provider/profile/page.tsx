...
import type { ProviderType } from "@/lib/api-schema";
...
export default function ProviderProfilePage() {
  ...
  const deleteServiceMut = useMutation({
    mutationFn: (id: string) => deleteProviderService(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["provider-services"] });
  ...
  const profilePatchMut = useMutation({
    ...
    }) => patchProviderProfile(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["provider-profile", "page"] });
  ...
  function formatRate(item: ProviderServiceOffering) {
  ...
  useEffect(() => {
    if (!user?.id) return;
    try {
      const raw = window.localStorage.getItem(`provider-alerts:${user.id}`);
      if (raw) {
      ...
      const rawTools = window.localStorage.getItem(`provider-tools:${user.id}`);
      if (rawTools) {
      ...
      const rawLocationLabel = window.localStorage.getItem(`provider-workshop-label:${user.id}`);
      if (rawLocationLabel) {
  ...
  useEffect(() => {
    ...
    // The two coordinates above are the only parts of `profile` this reads; depending on
    // the whole object would re-run on every refetch and overwrite the shown label.
  ...
  function saveAlertsSettings() {
    if (!user?.id) return;
    try {
      window.localStorage.setItem(`provider-alerts:${user.id}`, JSON.stringify(alertsSettings));
  ...
  function saveTools() {
    if (!user?.id) return;
    try {
      window.localStorage.setItem(`provider-tools:${user.id}`, JSON.stringify(tools));
          ...
          },
          {
            onSuccess: () => {
              if (user?.id) {
                ...
                window.localStorage.setItem(`provider-workshop-label:${user.id}`, label);
  ...
  return (
    <div className="px-4 py-6">
      <h1 className="font-sora text-5xl text-[#8df6ba]">Provider Profile</h1>

      {/* Above the fold on purpose: until a provider is verified, nothing else on this
          page changes whether they can work. */}
      <div id="verification" className="mt-4 scroll-mt-24">
        <VerificationPanel />
      </div>

      <div id="agency" className="mt-4 scroll-mt-24">
        <AgencyPanel />
      </div>

      {/* Trade, not role. A tow operator is not a mechanic, and until this existed every
          provider stayed `mechanic` — so tow-capable matching could never find them. */}
      <GlassCard className="mt-4 border-white/10 bg-[#1f2c3f]/90">
        <p className="text-[11px] uppercase tracking-[0.16em] text-white/45">What you do</p>
        <div className="mt-3 grid grid-cols-3 gap-2">
          {(
            [
              { value: "mechanic", label: "Repairs" },
            ...
            <button
              key={option.value}
              type="button"
              disabled={profilePatchMut.isPending}
              onClick={() => profilePatchMut.mutate({ provider_type: option.value })}
              className={cn(
                "rounded-xl border px-3 py-2 text-sm transition-colors disabled:opacity-60",
                profile?.provider_type === option.value
                  ? "border-[#00E676]/70 bg-[#00E676]/10 text-white"
                  : "border-white/15 bg-[#1b2739]/60 text-white/70 hover:border-white/30",
              )}
            >
              {option.label}
            </button>
        ...
        <p className="mt-2 text-xs text-white/40">
          Requests that need a vehicle moved are only offered to tow-capable providers.
        </p>
      </GlassCard>
        ...
        <GlassCard className="border-white/10 bg-[#253247]/90">
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-white/60">Provider account</p>
              ...
              <p className="mt-3 text-white/70">
                {profile?.bio?.trim() || "Add your bio so customers can understand your workshop specialization."}
              </p>
