...
import { IncomingRequestModal } from "@/features/provider/components/incoming-request-modal";
import { GlassCard } from "@/components/ui/glass-card";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { api } from "@/lib/api";
import { useRealtimeStore } from "@/store/realtime-store";
import { acceptJob, listJobs, listNearbyOpenRequests, cancelJob, type NearbyRequestItem } from "@/services/jobs";
import { asVerificationRequired, levelLabel } from "@/services/verification";
...
export default function ProviderHomePage() {
  ...
  const activeJobHref = activeJob ? `/provider/job/${activeJob.id}` : "/provider/history";
  ...
  const displayName = profile?.business_name?.trim() || "Provider";
  const queryCoords =
    coords ??
  ...
  const patchAvailability = useMutation({
    mutationFn: async (payload: { next: boolean; lat?: number; lng?: number }) => {
      ...
      const { data } = await api.patch("/providers/profile/", body);
      return data;
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    // `coords` is read to avoid clobbering a position the provider already has, but must not
    // re-trigger this effect — including it would immediately undo the value just set.
  ...
  async function setAvailability(next: boolean) {
    ...
    if (next) {
      ...
      if (typeof onlineLat !== "number" || typeof onlineLng !== "number") {
        toast.error("Enable location first so customers can find you on the map.");
        return;
      }
    }
    setOnline(next);
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1fr_360px]">
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/provider/history" className="text-sm text-[#8df6ba]">
              View schedule →
            </Link>
          </div>
              ...
              pendingJobs.slice(0, 3).map((job) => (
                <GlassCard key={job.id} className="border-white/10 bg-[#253247]/90 transition hover:-translate-y-0.5 hover:border-[#00E676]/40">
                  <div className="flex items-center justify-between gap-3">
                    <Link href={`/provider/job/${job.id}`} className="contents">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-white/60">
                          Customer: {job.customer_name || "Customer"} · Job #{job.id.slice(0, 8)}
                        </p>
                      </div>
                    </Link>
      ...
      <IncomingRequestModal
        ...
        request={
          visibleIncomingRequest
            ? {
                customerName: visibleIncomingRequest.customer_name || "Customer",
        ...
        onAccept={async () => {
          ...
          try {
            ...
            setStatus("live");
            router.replace(`/provider/job/${created.id}`);
          } catch (error) {
            // Three different outcomes wear the same generic message otherwise, and one of
            // them is a conversion moment rather than a failure.
            const gate = asVerificationRequired(error);
            if (gate) {
              toast.error(
                `Verification required to accept jobs — ${levelLabel(gate.required_level)} needed.`,
                {
                  action: {
                    label: "Verify",
                    onClick: () => router.push("/provider/profile#verification"),
                  },
                },
              );
              setIncoming(false);
            ...
            toast.error(
              status === 409
                ? "Another provider took it, it expired, or you are at your job limit."
