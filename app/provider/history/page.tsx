...
export default function ProviderHistoryPage() {
  ...
  const rejectMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["jobs", "provider-history"] });
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      if (!q) return true;
      const text = `${job.id} ${job.service_category_name ?? ""} ${job.customer_name ?? ""} ${job.status}`.toLowerCase();
  ...
  return (
    <div className="px-4 py-6">
      <p className="text-xs uppercase tracking-[0.16em] text-white/45">Provider hub</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <label className="inline-flex flex-1 items-center gap-2 rounded-xl border border-white/10 bg-[#0f1727] px-3 py-2 text-white/65">
          ...
          <input
            ...
            placeholder="Search by customer, category or job ID..."
            value={query}
          ...
          filtered.map((job) => (
            <GlassCard key={job.id} className="border-white/10 bg-[#253247]/90 !py-3 transition hover:border-[#00E676]/40">
              <div className="grid items-center gap-3 md:grid-cols-[1fr_220px_220px]">
                <Link href={`/provider/job/${job.id}`} className="min-w-0">
                  <div>
                    ...
                    <p className="text-sm text-white/65">Customer: {job.customer_name || "Customer"}</p>
                  </div>
                </Link>
                <p className="text-sm text-white/65">{job.created_at ? new Date(job.created_at).toLocaleString() : "Unknown date"}</p>
                <div className="flex items-center gap-2">
                  <p
                    className={`justify-self-start rounded-full px-3 py-1 text-xs uppercase tracking-wider ${statusBadgeClass(job.status ?? "pending_accept")}`}
                  >
