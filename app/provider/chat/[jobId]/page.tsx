...
export default function ProviderChatPage() {
  ...
  const jobsQ = useQuery({
    queryKey: ["jobs", "provider-chat", jobIdParam],
    queryFn: listJobs,
  ...
  // Every hook below must run on every render, so the early return moved to just above
  // the JSX. `jobId` is "" when the route param is missing and `hasJobId` gates the
  // fetching, so nothing requests a blank id in the meantime.
  const hasJobId = Boolean(jobIdParam);
  const jobId = jobIdParam ?? "";

  // Stable identity so the effects below can depend on it honestly rather than omitting it.
  ...
  useEffect(() => {
    ...
    // `user?.id` is reached only via `mapMessage`, which is already a dependency.
    // Adding it would tear down and reopen the socket on any identity refresh.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jobId, hasJobId, mapMessage]);
  ...
  const hasConnectedCustomers = contacts.length > 0;
  const currentContact =
    ...
    (jobId
      ? {
          ...
          job: jobId,
          contact_name:
            (jobsQ.data ?? []).find((j) => j.id === jobId)?.customer_name ||
            null,
          last_message: null,
        }
      : null);
  const canChatInCurrentThread = Boolean(jobId);
  const threadCustomerName = currentContact?.contact_name || "Customer";
        ...
        <GlassCard className="hidden h-full min-h-0 flex-col border-slate-300/70 bg-white/95 p-3 lg:flex dark:border-white/10 dark:bg-[#1a2437]/90">
          ...
          {hasConnectedCustomers ? (
            <div className="mt-3 flex-1 space-y-2 overflow-y-auto pr-1">
              {contacts.map((room) => {
                ...
                return (
                  ...
                  >
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">{room.contact_name || "Customer"}</p>
            ...
            <div className="mt-6 rounded-xl border border-dashed border-slate-300/70 bg-slate-50 p-4 text-center dark:border-white/10 dark:bg-white/5">
              ...
              <p className="mt-2 text-sm font-medium text-slate-800 dark:text-white/85">No connected customers</p>
        ...
        <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 dark:border-transparent dark:bg-transparent">
          <GlassCard className="mb-3 border-slate-300/70 bg-white/95 dark:border-white/10 dark:bg-[#253247]/90">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-sora text-3xl text-slate-900 dark:text-white">{threadCustomerName}</p>
                <p className="text-xs uppercase tracking-[0.16em] text-emerald-600 dark:text-[#8ef7bb]">Active thread</p>
              </div>
              <div className="flex items-center gap-1">
                <Link href="/provider/messages" className="lg:hidden">
