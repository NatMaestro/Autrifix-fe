...
export default function InProgressPage() {
  ...
  const jobsQ = useQuery({
    queryKey: ["jobs", "customer-in-progress"],
    queryFn: listJobs,
    staleTime: 10_000,
  });
  ...
  useEffect(() => {
    setActiveJob("/customer/in-progress", "Service in progress");
  }, [setActiveJob]);
    ...
    <div className="relative isolate h-dvh overflow-hidden">
      <div className="absolute inset-0 z-0">
        <AutriMap
          ...
          zoom={13}
          markers={[
            { id: "me", lat: C.lat, lng: C.lng, label: "You", subtitle: "Customer location" },
            {
              id: "mech",
              lat: M.lat,
              lng: M.lng,
      ...
      <div className="absolute left-4 top-20 z-10">
        <GlassCard className="!py-2 border-[#00E676]/30 bg-[#1a2a3b]/90">
          <p className="inline-flex items-center gap-2 font-medium text-[#8cf4b8]">
            <span className="h-2 w-2 rounded-full bg-[#00E676]" /> Provider is on-site
          </p>
        </GlassCard>
      </div>
      <div className="absolute bottom-24 left-4 right-4 z-10 grid gap-3 xl:bottom-6 xl:grid-cols-[360px_1fr]">
        ...
        {liveJob ? <JobMoneyPanel job={liveJob} /> : null}
