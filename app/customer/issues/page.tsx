...
import { DestinationPicker } from "@/components/jobs/destination-picker";
import { findCategory, nearbyServices, routeIssue } from "@/services/services";
...
export default function IssuesPage() {
  ...
  // Set while we ask for a destination. Holding the routed category avoids re-running the
  // classifier — and re-routing could conceivably land on a different category, which would
  // silently change what the customer is asking for.
  const [pendingDestination, setPendingDestination] = useState<{ categoryId: string } | null>(
    null,
  );
  ...
  async function submitRequest(destination: { lat: number; lng: number } | null) {
    const issueText = `${customIssue.trim()}${selected ? ` ${selected}` : ""}`.trim();
    try {
      const routed = await routeIssue(issueText);
      const categoryId = routed?.category_id ?? null;
      ...
      // Tow categories relocate the vehicle, so the backend requires a destination and
      // rejects the request without one. Ask for it rather than failing validation.
      const routedCategory = await findCategory(categoryId);
      if (routedCategory?.requires_destination && !destination) {
        setPendingDestination({ categoryId });
        return;
      }
      ...
      // The open-request cap is an ordinary outcome, not a failure (backend SPEC-016 REQ-4).
      // A generic message leaves the customer retrying something that will never succeed.
      const status = (err as { response?: { status?: number } })?.response?.status;
      if (status === 409) {
        toast.error(
          "You already have requests waiting for a provider. Cancel one before creating another.",
        );
        return;
      }
      ...
      {pendingDestination ? (
        <DestinationPicker
          ...
          onConfirm={(destination) => {
            ...
            void submitRequest(destination);
          }}
        />
      ) : null}
      ...
      <GlassCard className="mt-6 border-slate-300/60 bg-white/90 dark:border-white/10 dark:bg-[#1b273a]/80">
        <form onSubmit={save} className="space-y-5">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <Link href="/customer" className="sm:order-1">
