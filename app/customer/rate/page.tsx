...
import { createReview, isDuplicateReview } from "@/services/reviews";
...
export default function RatePage() {
  ...
  // A review is only legal on a completed job (backend ADR-011), and completion now means
  // the customer confirmed it — so this list is exactly what they may review.
  const reviewable = jobs.filter((j) => j.status === "completed");
  const job = (jobId ? jobs.find((j) => j.id === jobId) : null) ?? reviewable[0] ?? null;
  ...
  const createMut = useMutation({
    mutationFn: () =>
      createReview({
        job: job!.id,
        rating: stars,
        // Tags are a UI affordance the API has no field for, so they are folded into the
        // comment rather than silently discarded.
        comment: [comment.trim(), tags.length ? `(${tags.join(", ")})` : ""]
          .filter(Boolean)
          .join(" "),
      }),
    onSuccess: () => {
    ...
    onError: (error: unknown) => {
      ...
      toast.error("Could not send your review. Try again.");
    },
  });

  function submit() {
    if (!job) {
      toast.error("No completed job to review yet.");
      return;
    }
    if (stars < 1) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-sora text-2xl font-semibold text-white">
              {job?.provider_name ?? "Your provider"}
            </p>
