...
export default function ProvidersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return (nearby?.providers ?? []).filter((item) => {
      if (serviceFilter !== "all") {
        // nearby providers endpoint does not include per-provider service labels yet
        // so service filter is treated as category intent only.
      }
      if (q && !`${item.business_name}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [nearby?.providers, query, serviceFilter]);

  return (
    <div className="min-h-dvh px-4 pb-28 pt-8">
      <h1 className="font-sora text-5xl font-semibold text-white/90">Nearby providers</h1>
          ...
          <GlassCard className="border-white/10 bg-[#1b273a]/85">
            ...
            <div className="mt-4 space-y-4">
              <div className="flex items-center justify-between text-white/70">
                ...
                <span className="text-3xl font-semibold text-white">{nearby?.nearby_providers_count ?? 0}</span>
              </div>
              <div className="flex items-center justify-between text-white/70">
        ...
        <div className="space-y-3">
                ...
                >
                  <div>
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-sora text-3xl font-semibold text-white">{p.business_name || "Provider"}</p>
                  ...
                  <div className="text-right">
                    ...
                    <Link
                      href={`/customer/issues?source=${encodeURIComponent(p.id)}`}
          ...
          {!isLoading && !filtered.length ? (
            <GlassCard className="border-white/10 bg-[#1b273a]/80">
              <p className="text-white/70">No nearby providers found right now.</p>
            </GlassCard>
          ) : null}
        </div>
      </div>
