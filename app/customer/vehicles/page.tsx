...
function persistVehiclePackForProviderPreview(vehicles: VehicleDto[]) {
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      vehicle: primary,
      disclaimer:
        "Demo preview: provider job screen reads this local snapshot until job APIs attach real vehicle context.",
    }),
  );
}
...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    ...
    persistVehiclePackForProviderPreview(vehicles);
  }, [vehicles]);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600 dark:text-white/60 sm:text-base">
            ...
            <span className="text-slate-900 dark:text-white">primary vehicle context</span> helps providers bring compatible parts
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        ...
        <GlassCard className="border-slate-300/60 bg-white/90 dark:border-white/10 dark:bg-[#1b2739]/85">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-600 dark:text-white/65">
            ...
            <li>Provider-visible “parts checklist” per job category</li>
          </ul>
        </GlassCard>
      </div>
      ...
      {isError ? (
        <GlassCard className="mt-6 border-red-500/30 bg-red-950/30">
          ...
          <p className="mt-2 text-xs text-red-100/70">
            If you are not logged in as a customer, open <span className="text-white">/auth/login</span>.
          </p>
        </GlassCard>
      ) : null}
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-2">
        <GlassCard className="border-slate-300/60 bg-white/90 dark:border-white/10 dark:bg-[#1b2739]/85">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-white/65">
            ...
            Avoid posting personal documents, home/work routines, or anything beyond what the provider needs to safely
            assist you.
          </p>
        </GlassCard>
        <GlassCard className="border-slate-300/60 bg-white/90 dark:border-white/10 dark:bg-[#1b2739]/85">
    ...
    <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/60 p-3 sm:items-center">
      <GlassCard className="max-h-[90dvh] w-full max-w-2xl overflow-y-auto border-slate-300/70 bg-white/95 dark:border-white/10 dark:bg-[#15243c]/95 p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <Field label="Nickname (optional)" value={label} onChange={setLabel} placeholder="Daily customer" />
