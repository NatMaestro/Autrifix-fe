...
import { LiveRequestPanel } from "@/features/customer/components/live-request-panel";
import { useCustomerNearbyProvidersWs } from "@/hooks/use-customer-nearby-providers-ws";
...
export default function CustomerHomePage() {
  ...
  // Extracted so the dependency array holds plain values: a conditional expression inside
  // the array cannot be checked statically, and the rule cannot tell whether it is correct.
  const geoLat = geo.status === "ok" ? geo.lat : null;
  const geoLng = geo.status === "ok" ? geo.lng : null;
  ...
  useEffect(() => {
    ...
    // `geoLat`/`geoLng` above are these same values, narrowed; depending on both forms
    // would be redundant.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [geo.status, geoLat, geoLng, searchCenterLocked]);
  ...
  const { liveProviders } = useCustomerNearbyProvidersWs({
    center: liveCenter,
    radiusKm: 25,
    enabled: Boolean(user?.role === "customer"),
  });
  const providersForMap = useMemo(() => {
    const byId = new Map<string, NearbyProviderPreview>();
    for (const m of nearby?.providers ?? []) byId.set(m.id, m);
    for (const m of liveProviders ?? []) byId.set(m.id, m);
    return Array.from(byId.values()).sort((a, b) => a.distance_km - b.distance_km);
  }, [nearby?.providers, liveProviders]);
  const markerReadyProviders = useMemo(
    () =>
      providersForMap.filter(
        (m) =>
  ...
  const nearbyCount = markerReadyProviders.length;
  const httpCount = nearby?.providers?.length ?? 0;
  const wsCount = liveProviders?.length ?? 0;
  const showDiagnostics = searchParams.get("diag") === "1";
  const liveProviderMarkers = useMemo(() => {
    const buckets = new Map<string, number>();
    return markerReadyProviders.slice(0, 25).map((m) => {
      ...
      if (overlapIndex === 0) {
        return {
          ...
          lat: m.latitude,
          lng: m.longitude,
          label: m.business_name || "Provider",
          subtitle: `Provider · ${m.rating_avg.toFixed(1)} ★ · ${m.distance_km.toFixed(1)} km away`,
        };
      }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden rounded-2xl border border-slate-300/70 bg-white/85 px-3 py-1.5 text-xs text-slate-700 backdrop-blur-md dark:border-white/10 dark:bg-black/30 dark:text-white/70 sm:block">
            <span className="text-[#00E676]">
              {nearby || liveProviders !== null ? String(nearbyCount) : "—"}
            </span>{" "}
            nearby technicians
            {liveProviders !== null ? (
      ...
      <div className="absolute inset-x-0 bottom-8 z-[70]">
        <LiveRequestPanel
          onRequest={async (issue, tag) => {
            ...
            if (!categoryId) {
              // Last-resort fallback so customer flow still proceeds even if AI route
            ...
            setActiveJob("/customer/matching", "Finding provider");
