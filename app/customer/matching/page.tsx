...
export default function MatchingPage() {
  ...
  useEffect(() => {
    ...
    toast.success("Provider found. Heading to live tracking.");
    router.replace(`/customer/track?jobId=${matchedJob.id}`);
  }, [
    matchedJob,
    pendingRequestId,
    router,
  ...
  useEffect(() => {
    ...
    setPendingRequestServiceLabel(null);
    setPendingRequestRadiusKm(null);
    toast.error("No available provider at this time. Please try again later.");
    const t = window.setTimeout(() => router.replace("/customer"), 2200);
      ...
      <GlassCard className="w-full max-w-2xl border-slate-300/70 bg-white/95 p-8 text-center dark:border-white/10 dark:bg-[#151f30]/80">
        ...
        <div className="mt-4">
          <h1 className="font-sora text-5xl font-semibold text-slate-900 dark:text-white">
            {timedOut ? "No available provider right now" : "Finding the best help near you..."}
          </h1>
          <p className="mx-auto mt-3 max-w-2xl text-xl text-slate-600 dark:text-white/60">
            {timedOut
              ? "No available provider at this time. Please try again later."
              : "Connecting with top-rated providers and roadside assistance in your area."}
          </p>
        </div>
          ...
          <div className="space-y-3 text-sm">
            ...
            <p className="flex items-center justify-between text-slate-700 dark:text-white/75">
              <span className="inline-flex items-center gap-2">
                ...
                {timedOut ? "No provider accepted in time" : "Optimizing response time"}
              </span>
              {timedOut ? (
        ...
        <div className="mt-8 flex w-full justify-center gap-3">
          <Button
            ...
            size="lg"
            disabled={!matchedJob}
            onClick={() => router.push(`/customer/track${matchedJob?.id ? `?jobId=${matchedJob.id}` : ""}`)}
          >
